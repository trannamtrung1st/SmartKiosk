...
{
    public static class EntityCategoryQuery
    {
        public static IQueryable<EntityCategory> Id(this IQueryable<EntityCategory> query, int id)
        {
            return query.Where(o => o.Id == id);
        }

        public static IQueryable<EntityCategory> IdOnly(this IQueryable<EntityCategory> query)
        {
            return query.Select(o => new EntityCategory { Id = o.Id });
        }

        public static bool Exists(this IQueryable<EntityCategory> query, int id)
        {
            return query.Any(o => o.Id == id);
        }

        public static IQueryable<EntityCategory> Ids(this IQueryable<EntityCategory> query, IEnumerable<int> ids)
        {
            ...
            EntityCategoryQuerySort model)
        {
            ...
            foreach (var s in model._sortsArr)
            {
                ...
                switch (fieldName)
                {
                    case EntityCategoryQuerySort.NAME:
                        {
                            listSorts.Add($"{nameof(EntityCategoryContent)}" +
                                $".{nameof(EntityCategoryContent.Name)}{(asc ? "" : " DESC")}");
                        }
                        break;
                }
            }
        ...
        public static DynamicSql SqlFilter(
            this DynamicSql query, EntityCategoryQueryFilter filter)
        {
            query = DynamicSql.DeepClone(query);
            ...
            if (filter.id != null)
            {
                ...
                listFilters.Add($"{nameof(EntityCategory)}.{nameof(EntityCategory.Id)}=@{paramName}");
            }
            if (filter.name_contains != null)
            {
                var paramName = query.AddAutoIncrParam(filter.name_contains);
                listFilters.Add($"CHARINDEX(@{paramName}, {nameof(EntityCategoryContent)}" +
                    $".{nameof(EntityCategoryContent.Name)}) > 0");
            }
            if (filter.archived != 2)
            {
                var paramName = query.AddAutoIncrParam(filter.archived);
                listFilters.Add($"{nameof(EntityCategory)}.{nameof(EntityCategory.Archived)}=@{paramName}");
            }
            if (listFilters.Any())
            {
            ...
            this DynamicSql query, EntityCategoryQueryProjection model)
        {
            ...
            var finalFields = model.GetFieldsArr()
                .Where(f => EntityCategoryQueryProjection.Projections.ContainsKey(f))
                .Select(f => EntityCategoryQueryProjection.Projections[f]);
            if (finalFields.Any())
            {
            ...
            var finalResults = model.GetFieldsArr()
                .Where(f => EntityCategoryQueryProjection.Results.ContainsKey(f))
                .Select(f => EntityCategoryQueryProjection.Results[f]);
            ...
            EntityCategoryQueryFilter filter)
        {
            ...
            var joins = model.GetFieldsArr()
                .Where(f => EntityCategoryQueryProjection.Joins.ContainsKey(f))
                .Select(f => EntityCategoryQueryProjection.Joins[f]);
            if (joins.Any())
            {
                ...
                if (filter != null)
                {
                    ...
                    if (filter.lang != null)
                    {
                        ...
                        var postContentLang = $"{nameof(EntityCategoryContent)}.{nameof(EntityCategoryContent.Lang)}";
                    ...
                    if (contentFilters.Any())
                    {
                        ...
                        query.DynamicForm = query.DynamicForm
                            .Replace(EntityCategoryQueryPlaceholder.EC_CONTENT_FILTER, whereClause);
                    }
                }
            }
            return query;
        ...
        public static DynamicSql CreateDynamicSql()
        {
            var sql = $"SELECT {DynamicSql.PROJECTION} " +
                $"FROM {nameof(EntityCategory)} as {nameof(EntityCategory)}\n" +
                $"{DynamicSql.JOIN}\n" +
                $"{DynamicSql.FILTER}\n" +
                $"{DynamicSql.GROUP}\n" +
                $"{DynamicSql.SORT}\n" +
